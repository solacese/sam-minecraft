...
export function registerFurnaceTools(factory: ToolFactory, getBot: () => mineflayer.Bot): void {
  factory.registerTool(
    ...
    async (params: {
      ...
      const inputItem = params.inputItem;
      const inputCount = params.inputCount ?? 1;
      const fuelItem = params.fuelItem;
      const fuelCount = params.fuelCount ?? 1;
      const takeOutput = params.takeOutput ?? true;
