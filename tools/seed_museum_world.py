...
def prepare_plot(center_x: int, center_z: int, radius: int, top_y: int = 180) -> list[str]:
    ...
    commands.extend(fill_commands(x1, BASE_Y - 3, z1, x2, BASE_Y - 3, z2, "stone"))
    commands.extend(fill_commands(x1, BASE_Y - 2, z1, x2, BASE_Y - 2, z2, "dirt"))
...
def place_ots(path: pathlib.Path, center_x: int, center_z: int, title: str) -> list[str]:
...
def seed_spec(spec_id: str, center_x: int, center_z: int, scale: str = "medium") -> list[str]:
    ...
    for component in spec["components"]:
        ...
        if tool == "flatten-area":
            ...
            commands.extend(garden_commands(cx, cz, int(component.get("gardenSize", 2))))
...
def garden_commands(cx: int, cz: int, size: int) -> list[str]:
    radius = max(3, size * 3)
...
def paths_and_spawn() -> list[str]:
    commands: list[str] = []
    commands.extend(fill_commands(-260, SURFACE_Y, -4, 270, SURFACE_Y, 4, "polished_andesite"))
    commands.extend(fill_commands(-4, SURFACE_Y, -120, 4, SURFACE_Y, 280, "polished_andesite"))
    commands.extend(fill_commands(-12, SURFACE_Y, -120, 12, SURFACE_Y, -96, "polished_andesite"))
    commands.extend([
        ...
        "setworldspawn 0 69 -108",
        "spawnpoint @a 0 69 -108",
        "tp @a 0 70 -112 0 10",
    ])
    return commands
...
def main() -> int:
    ...
    for center_x, center_z, radius in exhibits:
        all_commands.extend(prepare_plot(center_x, center_z, radius))
    if not args.skip_ots:
        all_commands.extend(place_ots(REPO_ROOT / "vendor/minecraft-mcp-server/local_structures/munich_famous_building.ots_blocks", 0, 0, "Munich Famous Building"))
        all_commands.extend(place_ots(REPO_ROOT / "vendor/minecraft-mcp-server/local_structures/sydney_opera_house_cadnav.ots_blocks", 0, 220, "Sydney Opera House"))
    if not args.skip_specs:
        all_commands.extend(seed_spec("eiffel_tower_fr", 220, 0))
        all_commands.extend(seed_spec("colosseum_it", 220, 220, "small"))
        all_commands.extend(seed_spec("neuschwanstein_castle_de", -220, 220, "small"))
        all_commands.extend(seed_spec("tower_of_pisa_it", -220, 0, "small"))
    all_commands.extend(paths_and_spawn())
